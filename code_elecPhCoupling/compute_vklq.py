...
def read_coeffs(nElec, nHole, nExc):
    ...
    c = (c[:nExc*nElec*nHole]).astype(float)
    return c
...
def read_mass(natoms):
    ...
    ga_idx = np.where(at_type == 'Ga')[0]
    as_idx = np.where(at_type == 'As')[0]
    in_idx = np.where(at_type == 'In')[0]
    p_idx = np.where(at_type == 'P')[0]

    if (cd_idx.shape[0] + se_idx.shape[0] + s_idx.shape[0] + ga_idx.shape[0] + as_idx.shape[0] + in_idx.shape[0] + p_idx.shape[0]!= natoms):
        print('Error reading ' + filename + '! Exiting...\n')
        exit()
    mass[cd_idx] = 112.411
    mass[se_idx] = 78.960
...
def read_phononModes(natoms):
    ...
    pmodes = np.array([line.strip().split() for line in open(filename, 'r')]).astype(float) #.transpose()
...
def read_DiabCoupling(natoms, nQPs, Vrs_file):
    ...
    qp1Idx = np.array([line[6] for line in lines]).astype(int)
    qp2Idx = np.array([line[8] for line in lines]).astype(int)
    ...
    Vrsx = np.array([line[10] for line in lines]).astype(float).reshape((natoms, nQPs, nQPs))
    Vrsy = np.array([line[11] for line in lines]).astype(float).reshape((natoms, nQPs, nQPs))
    Vrsz = np.array([line[12] for line in lines]).astype(float).reshape((natoms, nQPs, nQPs))
...
def compute_excCouplingAtomic(natoms, nElec, nHole, nExc):
    ...
    if os.path.exists('Vkl-diabatic.dat'):
        Vkl = np.array([line.strip().split()[-1] for line in open('Vkl-diabatic.dat', 'r')]).astype(float)
        # check if excitonic diabatic couplings computed for nExc
        idx = np.sqrt(Vkl.shape[0]/(3*natoms)).astype(int)
        if idx >= nExc:
            Vkl = Vkl.reshape((3*natoms, idx, idx))
            return Vkl
        else:
            Vkle = np.zeros((3*natoms, nExc, nExc))
            tmp = np.array([line.strip().split()[-1] for line in open('Vkle-diabatic.dat', 'r')]).astype(float)
            Vkle[:,:idx,:idx] = tmp.reshape((3*natoms, idx, idx))

            Vklh = np.zeros((3*natoms, nExc, nExc))
            tmp = np.array([line.strip().split()[-1] for line in open('Vklh-diabatic.dat', 'r')]).astype(float)
...
def compute_excCouplingPhonon(natoms, nElec, nHole, nExc):
    ...
    if os.path.exists('Vklq-diabatic.dat'):
        Vklq = np.array([line.strip().split()[-1] for line in open('Vklq-diabatic.dat', 'r')]).astype(float)
        # check if excitonic diabatic couplings computed for nExc
        idx = np.sqrt(Vklq.shape[0]/(3*natoms)).astype(int)
        if idx >= nExc:
